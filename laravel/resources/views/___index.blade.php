                ...
                <div class="bg-gradient-to-r from-blue-600 to-purple-600 rounded-2xl p-6 mb-6 text-center">
                    ...
                    <div class="mt-3 flex justify-center">
                        <div class="bg-white/20 px-4 py-2 rounded-full">
                            <span id="presensi-status" class="text-sm font-medium"></span>
                        </div>
                    </div>
                </div>
        ...
        function processFrame() {
            ...
            setInterval(async () => {
                ...
                if (resizedDetections.length > 0) {
                    ...
                    document.getElementById('presensi-status').textContent = 'Menunggu deteksi wajah';
                    document.getElementById('scan-progress').style.width = '0%';
                }
        ...
        async function scan(detections) {
            ...
            tempCanvas.toBlob(async (blob) => {
                ...
                try {
                    ...
                    if (data.status) {
                        successfulScans++;
                        speak(`Halo ${data.nama}, presensi ${data.message}`, maleVoice);
                        showUser(data.nama);
                        logAttendance(data.nama, 'success', data.message);
                        showNotification('success', 'Presensi Berhasil', `Presensi ${data.nama} sukses!`);
                        document.getElementById('presensi-status').textContent =
                        `Presensi Berhasil: ${data.nama}`;
                        document.getElementById('presensi-status').className =
                            "bg-green-100 text-green-700 px-4 py-2 rounded-full font-semibold shadow-md";
                    } else {
                        speak(data.message || "Presensi gagal.", femaleVoice);
                        showUser("Tidak Dikenali", true);
                        logAttendance("Gagal", 'error', data.message || "Wajah tidak dikenali.");
                        showNotification('error', 'Presensi Gagal', data.message || "Wajah tidak dikenali.");
                        document.getElementById('presensi-status').textContent =
                        `Presensi Gagal: ${data.message}`;
                        document.getElementById('presensi-status').className =
                    ...
                    document.getElementById('presensi-status').textContent = `Error: ${error.message}`;
                    document.getElementById('presensi-status').className =
                        "bg-red-100 text-red-700 px-4 py-2 rounded-full font-semibold shadow-md";
                }
            }, 'image/jpeg', 0.8);
