...
@section('page-title', 'Aturan Presensi')
        ...
        <div class="card-body">
            ...
            <div class="table-responsive">
                <table id="table" class="table table-striped table-bordered text-nowrap align-middle">
                    <thead>
                        <tr>
                            <th colspan="2" class="text-center">Presensi Masuk</th>
                            <th colspan="2" class="text-center">Presensi Pulang</th>
                    ...
                    <tbody>
                        @forelse ($data as $item)
                            <tr>
                                {{-- MASUK --}}
                                <td>{{ $item['masuk']?->start_time ? \Carbon\Carbon::parse($item['masuk']->start_time)->format('H:i') : '-' }}
                                    {{-- <td>{{ $item['masuk']?->end_time ? \Carbon\Carbon::parse($item['masuk']->end_time)->format('H:i') : '-' }} --}}
                                <td>{{ $item['masuk']?->late_after ? \Carbon\Carbon::parse($item['masuk']->late_after)->format('H:i') : '-' }}

                                    {{-- PULANG --}}
                                <td>{{ $item['pulang']?->start_time ? \Carbon\Carbon::parse($item['pulang']->start_time)->format('H:i') : '-' }}
                                    {{-- <td>{{ $item['pulang']?->end_time ? \Carbon\Carbon::parse($item['pulang']->end_time)->format('H:i') : '-' }} --}}
                                <td>{{ $item['pulang']?->late_after ? \Carbon\Carbon::parse($item['pulang']->late_after)->format('H:i') : '-' }}
                                </td>
