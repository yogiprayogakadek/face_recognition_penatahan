...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <h5 class="card-title fw-semibold mb-4">Daftar Presensi</h5>
                ...
                <table id="table" class="table table-striped table-bordered text-nowrap align-middle" width="100%">
                    <thead>
                        <tr>
                            ...
                            <th colspan="2" class="text-center">Presensi Hari Ini</th>
                    ...
                    <tbody>
                        @foreach ($pegawai as $item)
                            ...
                            <tr>
                                ...
                                <td>{{ $item['presensi_masuk']?->checked_in_at ? \Carbon\Carbon::parse($item['presensi_masuk']->checked_in_at)->format('H:i') : '-' }}
                                <td>{{ $item['presensi_pulang']?->checked_in_at ? \Carbon\Carbon::parse($item['presensi_pulang']->checked_in_at)->format('H:i') : '-' }}
                                    {{-- <td>{{ $item['presensi_masuk']?->checked_in_at ?? '-' }}</td>
                                <td>{{ $item['presensi_pulang']?->checked_in_at ?? '-' }}</td> --}}
        ...
        <div class="modal-dialog modal-xl modal-dialog-scrollable" role="document">
            <div class="modal-content">
                <div class="modal-header d-flex align-items-center">
                    <h4 class="modal-title">Histori Presensi</h4>
...
@push('script')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('body').on('click', '.histori-btn', function() {
                let nama = $(this).data('nama');
                let id = $(this).data('id');
                let url = "{{ url('/presensi/histori') }}/" + id;

                $('#modal').modal('show');

                $('.modal-title').text('Histori Presensi - ' + nama);
