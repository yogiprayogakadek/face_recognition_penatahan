        ...
        try {
            ...
            if ($ruleMasuk && $nowTime >= $ruleMasuk->start_time && $nowTime <= $ruleMasuk->end_time) {
                ...
                if ($rulePulang->end_time && $nowTime > $rulePulang->end_time) {
                    return response()->json([
                        'status' => false,
                        'message' => 'Waktu presensi pulang sudah melewati batas (hingga ' . Carbon::parse($rulePulang->end_time)->format('H:i') . ' WITA).'
                    ]);
                }
                $tipe = 'pulang';
            }
            ...
            if (!$tipe) {
                ...
                $message = 'Belum waktunya presensi. Waktu presensi ' . implode(' dan ', $messageParts) . ' WITA.';
            ...
            $already = Presensi::where('pegawai_id', $data['pegawai_id'])
                ->where('tipe', $tipe)
                ->where('tanggal_presensi', $today) // Memastikan presensi hari ini
                ->exists();

            if ($already) {
                return response()->json([
                    'status' => false,
                    'message' => 'Anda sudah presensi ' . $tipe . ' hari ini.'
                ]);
            }
            ...
            Presensi::create([
                ...
                'aturan_presensi_id' => $activeRule->id ?? null, // Aturan mungkin null jika tidak ada
                'tipe' => $tipe,
                'checked_in_at' => $now,
                'is_late' => $isLate,
                'tanggal_presensi' => Carbon::now()->setTimezone('Asia/Makassar')->toDateString() // Simpan tanggal saja
            ]);
